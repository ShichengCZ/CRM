	...
	<script type="text/javascript">
		...
		$(function () {
			...
			$("#search-activity").keydown(function (event) {
				if (event.keyCode == 13){
				    var content = $.trim($("#search-activity").val())
				    $.ajax({
			...
			$("#search-contacts").keydown(function (event) {
				if (event.keyCode == 13){
                    ...
                    var contactsName = $.trim($("#search-contacts").val())
                    $.ajax({
            ...
            $("#saveContactsBtn").click(function () {
                ...
                var contactsId = $("input[name=xz2]:checked").val();
                var contactsName = $("#" + contactsId).html();
        ...
        //为阶段的下拉框，绑定选中下拉框的事件，根据选中的阶段填写可能性
            $("#create-stage").change(function () {
                ...
                /*
                    ...
                    阶段有了stage
                    阶段和可能性之间的对应关系pMap,但是pMap是java语言中的键值对的键值对关系
                    我们首先将pMap转换成js的键值对关系json

                    要做的是将pMap.put("01资质审查",10);
                    要做的是将pMap.put("02需求分析",20);
                    ...
                    转换为

                    var json = {"01资质审查":10,"02需求分析":25}

                    json已经处理好了，下面可以取可能性了
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
				...
				<div style="position: relative; top: -40px; left: 70%;">
					<button type="button" class="btn btn-primary" id="createBtn">保存</button>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input id="search-contacts" type="text" class="form-control" style="width: 300px;" placeholder="请输入联系人名称，支持模糊查询">
						    <span class="glyphicon glyphicon-search form-control-feedback"></span>
						  </div>
						</form>
					</div>
